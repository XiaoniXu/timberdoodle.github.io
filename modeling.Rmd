...
We applied grid operations to the map of New York State, dividing it into square grids to more precisely identify bird observation locations and simulate real-world conditions. After multiple adjustments, a grid size of 0.1 was selected as the optimal scale.
...
The observation probabilities of the American woodcock are displayed in the form of a heat map.
...
From the previous table, we may summarize the three optimal models suggested by each algorithms:
...
* **Conclusion:****Removing `landcover` simplifies the model without affecting accuracy or interpretability.**  
    - By excluding `landcover`, the model focuses on dynamic, measurable predictors such as temperature and snow for improving model robustness and ecological relevance, which are not only more ecologically relevant but also exhibit stronger predictive power. This approach enhances model efficiency while maintaining its ability to capture critical environmental influences on American woodcock activity.
